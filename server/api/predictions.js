    ...
    router.get('/glucose', async (req, res) => {
        ...
        try {
            ...
            const formattedHistory = allLogsInWindow.map(log => {
                ...
                const safeFoodName = log.foodName || 'Food';
                const numericAmount = Number(log.amount); // Ensure amount is a number

                if (log.type === 3) {
                    return `> At ${logTime}, glucose reading was ${Math.round(numericAmount)} mg/dL (Tag: ${log.tag || 'N/A'}).`;
                }
                if (log.type === 1) {
                    return `> At ${logTime}, user consumed "${safeFoodName}" (${Math.round(numericAmount)} kcal).`;
                }
                if (log.type === 2) {
                    return `> The meal "${safeFoodName}" at ${logTime} also contained ${numericAmount.toFixed(1)}g of sugar.`;
                }
                return '';
            }).join('\n');
            ...
            res.json({ success: true, ...predictionData, projectedTime: projectionTime.toISOString(), lastReading: { ...lastReading, amount: Math.round(Number(lastReading.amount)) } });
        } catch (error) {
